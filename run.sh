...
qrun -batch -access=rw+/. -uvmhome uvm-1.2 '-coverage' '-voptargs=+cover' -mfcu design.sv testbench.sv '-coverage' '-voptargs=+cover'  -do  " run -all; exit"  ; echo 'Creating result.zip...' && zip -r /tmp/tmp_zip_file_123play.zip . && mv /tmp/tmp_zip_file_123play.zip result.zip